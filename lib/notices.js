...
class NoticeService {
  ...
  async resend({ klass, noticeId, actor, ip }) {
    ...
    // “再次发送”是一轮新的展示。旧留言的绝对 expiresAt 往往已经过去，
    // 若直接复用会让大屏收到后立即清屏；按原持续时长从现在重新计时。
    const fresh = {
      ...notice,
      // 清掉旧的绝对到期时间，让 DisplayQueue 从真正开始展示的时刻计时。
      expiresAt: null,
      priority: notice.type === 'call' ? PRIORITY.CALL : notice.priority,
    };
    ...
    await this.store.update((db) => {
      const n = db.notices.find((x) => x.id === noticeId);
      if (n) { n.deliveryCount += 1; n.lastDeliveredAt = deliveredAt; n.status = 'published'; }
