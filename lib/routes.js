...
function createRouter({ store, classes, sessions, users, notices, scheduler, windows, publicDir }) {
  ...
  add('GET', '/api/admin/overview', admin((req, res) => {
    ...
    const displays = db.classes.filter((c) => c.status === 'active').map((c) => ({ classId: c.id, className: c.name, code: c.code, color: c.color, ...classes.counts(c.id), current: (classes.get(c.id) || { display: { snapshot: () => null } }).display.snapshot() }));
    sendOk(res, {
